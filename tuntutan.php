...
$select = $pdo->prepare("SELECT * FROM tuntut INNER JOIN ahli_kariah ON tuntut.kariah_id = ahli_kariah.kariah_id INNER JOIN penama ON tuntut.kariah_id = penama.kariah_id WHERE tuntut.kariah_id = $id");
$select->execute();
$row = $select->fetch(PDO::FETCH_OBJ);

$select1 = $pdo->prepare("SELECT tuntut.jumlah FROM tuntut INNER JOIN ahli_kariah ON tuntut.kariah_id = ahli_kariah.kariah_id INNER JOIN penama ON tuntut.kariah_id = penama.kariah_id WHERE tuntut.kariah_id = $id");
$select1->execute();
...
$html = '
	...
	<tr>
	<td>
	<strong>SI MATI</strong> : <span style="text-transform:uppercase">' . $row->tuntut_name . '</span><br/><br><strong>PENUNTUT</strong> : <span style="text-transform:uppercase">' . $row->penama_name . '</span><br><br><strong>TARIKH MENINGGAL</strong>: ' . date('d-m-Y', strtotime($row->tarikh_mati)) . '<br><br><strong>HUBUNGAN DENGAN SI MATI</strong> : <span style="text-transform:uppercase">' . $row->hubungan . '</span>
	</td>
	<td align="right">
	<strong>NO K/P</strong> : ' . $row->tuntut_ic . ' <br/><br>
...
<h4><strong>PENYATA RASMI TUNTUTAN KHAIRAT KEMATIAN</strong></h4>
';
$pdf->writeHTML($html, true, false, true, false, '');
...
$pdf->Output('penyata_tuntutan.pdf', 'I');
