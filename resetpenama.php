...
if ($stmt = mysqli_prepare($conn, 'SELECT penama_ic, penama_email, penama_token FROM penama WHERE penama_token= ?')) {
    mysqli_stmt_bind_param($stmt, "s", $token);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $penama_ic, $penama_email, $token);
    ...
    if (isset($_POST['btn_update_pass'])) {
        ...
        if ($penama_pass === $confirm_pass) {
            ...
            $hashpassword = password_hash($penama_pass, PASSWORD_BCRYPT, array('cost' => 12));

            if ($stmt = mysqli_prepare($conn, "UPDATE penama SET penama_token= '', penama_pass='{$hashpassword}' WHERE penama_email = ?")) {
...
<body class="d-flex align-items-center min-h-100 bg-dark">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <form class="g-py-15" method="POST">
                                            ...
                                            <div class="text-center">
                                                <div class="mb-5">
                                                    <h3 class="card-title">Kemaskini Kata Laluan Penama</h3>
                                                </div>
                                            </div>
                                            <div class="mb-4">
                                                <label class="form-label" for="signupSrPassword" tabindex="0">Kata Laluan</label>
                                                <div class="input-group-merge">
                                                    <input type="password" class="js-toggle-penama_pass form-control form-control-lg" name="penama_pass" id="penama_pass" placeholder="" aria-label="Kata Laluan" required="" oninvalid="this.setCustomValidity('Masukkan Kata Laluan')" oninput="setCustomValidity('')" data-hs-toggle-password-options='{
    ...
    <script>
        ...
        function myFunction() {
            var x = document.getElementById("penama_pass");
            if (x.type === "penama_pass") {
                x.type = "text";
            } else {
                x.type = "penama_pass";
            }
    ...
    <script>
        (function() {
            ...
            // =======================================================
            new HSTogglePassword(".js-toggle-penama_pass");
